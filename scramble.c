...
int main(int argc, string argv[])
{
    ...
    while (true)
    {
        ...
        if (strcmp(s, "SCRAMBLE") == 0 || strcmp(s, "scramble") == 0)
            scramble();
...
bool lookup(string s)
{
    // TODO
     ...
     {
        if (strcmp(dictionary.words[i].letters, s) == 0 && dictionary.words[i].found == false)
        {   
...
void scramble(void)
{
    ...
    // I put the rotated grid in a new temporary array 
    for (int i = 0; i < DIMENSION; i++)
    {
        for (int j = 0; j < DIMENSION  ; j++)
        {   
    ...
    //now a simple copy of the temp array to rewriting the original grid 
    for (int i = 0 ; i < DIMENSION ; i++)
    {
        for (int j = 0; j < DIMENSION ; j++)
        {
